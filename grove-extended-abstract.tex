...
A user actions is a high level operation that the formal semantics interprets as one or more graph edits.
The Grove calculus supports three user actions: Construct, Delete, and
Reposition.
For example, Alice moves the numer 1 from left to right by placing her cursor on the number 1 and issuing a Reposition action that describes the intended destination.
The following rule interprets Alice's action as two graph edits:
one edge is deleted and another is made live.
%
\begin{mathpar}
  \inferrule[Reposition]{
    ...
    \applyAction{
      ...
      \graphEdit{\Plus}{(\fresh{u}, v, p, v_{G_t})}
    }
  }
\end{mathpar}
%
Here, $v_{G_t}{=}(u_t, k_{G_t})$ is the vertex corresponding to $t$,
and $G_t$ is the subgraph of $G$ of all incoming edges to $v_{G_t}$,
