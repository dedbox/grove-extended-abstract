...
% Structural edits are a record of how a program has changed and having access to the record can be useful.
% Text diffs discard the record of changes.
% Git, for example, records the movement of a multi-line function as a sequence of independent line deletions and additions.
...
% (We're not doing the text diff thing because that throws away the record of what changed, and having access to that record directly, as opposed to inferring it through diffs, (??) .)


% (There are a few ways to handle this: e.g., git-style diff-and-merge is too lossy.
% Instead, represent changes as actions.)

% (There are other systems that use action logs, e.g., live share, but edits are still text based and we want them to be structure edits instead.)
